...
def test_training_storage_writes_and_reads_artifacts(tmp_path: Path) -> None:
    ...
    walkforward_report = {"model_name": "baseline_hist_gbr", "fold_count": 3}
    ...
    assert (
        ...
        == tmp_path / "reports" / "baseline_hist_gbr_walkforward_summary.json"
    )
    assert (
        walkforward_folds_path == tmp_path / "reports" / "baseline_hist_gbr_walkforward_folds.csv"
    )
    assert (
        walkforward_predictions_path
        == tmp_path / "predictions" / "baseline_hist_gbr_walkforward_test_predictions.csv"
    )
    assert storage.read_metrics_report("baseline_hist_gbr") == metrics_report
    assert storage.read_walkforward_summary("baseline_hist_gbr") == walkforward_report
